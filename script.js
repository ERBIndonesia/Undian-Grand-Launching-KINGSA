...
const STORAGE_KEY = "kingsa-slot-undian-v1";
...
function bindEvents() {
  ...
  el.noRepeatWinner.addEventListener("change", updateUI);
...
function availableNames() {
  if (!el.noRepeatWinner.checked) return [...state.participants];
  const winnerNames = new Set(state.winners.map((winner) => normalizeName(winner.name)));
  return state.participants.filter((name) => !winnerNames.has(normalizeName(name)));
}
...
function renderReel(names) {
  ...
  el.reelList.style.transition = "none";
  el.reelList.style.transform = "translateY(-10px)";
  requestAnimationFrame(() => {
    el.reelList.style.transition = "transform 0.08s linear";
    el.reelList.style.transform = "translateY(0)";
  });
}
...
function randomNamesForReel(finalWinner = null, forceFinal = false) {
  ...
  const pool = state.participants.length ? state.participants : defaultReelNames();
  const names = [];
